function renderHeader() {
  ...
  const cartItems = JSON.parse(localStorage.getItem("egaCart") || "[]");
  const cartCount = cartItems.reduce((sum, item) => sum + Number(item.quantity || 1), 0);
  const accountHtml = loggedInUser
    ? `
      <div class="dropdown">
        <a class="nav-link me-3 dropdown-toggle d-flex align-items-center" href="#" role="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Tài khoản">
          <i class="bi bi-person me-1"></i>
          <span class="fw-semibold">${loggedInUser.hoTen || "Tài khoản"}</span>
        </a>
        <ul class="dropdown-menu dropdown-menu-end">
          <li><a class="dropdown-item" href="#">Xin chào, ${loggedInUser.hoTen || "bạn"}</a></li>
          <li><a class="dropdown-item" href="../HTML/lich-su-mua-hang.html"><i class="bi bi-receipt me-2"></i>Lịch sử mua hàng</a></li>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <a href="../HTML/cart.html" class="nav-link position-relative" aria-label="Giỏ hàng">
                      ...
                      <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="display: ${cartCount > 0 ? 'inline-flex' : 'none'};">${cartCount}</span>
                    </a>
                </div>
            </div>
        </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".card").forEach((card) => {
    ...
    if (!cardImage || !btnContainer) return;

    cardImage.addEventListener("mouseover", () => {
      btnContainer.style.opacity = "1";
      btnContainer.style.transform = "translate3d(0%, -100%, 0)";
    });

    cardImage.addEventListener("mouseout", (event) => {
      if (!btnContainer.contains(event.relatedTarget)) {
        btnContainer.style.opacity = "0";
        btnContainer.style.transform = "translate3d(0%, 0%, 0)";
      }
    });
  });
});
